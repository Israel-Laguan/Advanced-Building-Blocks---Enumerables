...
RSpec.describe Enumerable do
  ...
  describe '#inject' do
    context 'is identical to #inject' do
      it 'using block, without proc' do
        test = test_arr.inject { |result, elem| result + elem }
        expect(test_arr.my_inject { |result, elem| result + elem }).to eq(test)
      end

      it 'using block with proc' do
        operation = proc { |sum, n| sum + n }
        test = array_of_numbers.inject(&operation)
        expect(array_of_numbers.my_inject(&operation)).to eq(test)
      end

      it 'without block, just Symbol' do
        test = test_arr.inject(:+)
        expect(test_arr.my_inject(:+)).to eq(test)
      end
      it 'without block, asing both arguments ' do
        test = range.inject(2, :*)
        expect(range.my_inject(2, :*)).to eq(test)
      end

      it 'using block and numeric argument' do
        test = range.inject(4) { |prod, n| prod * n }
        expect(range.my_inject(4) { |prod, n| prod * n }).to eq(test)
      end
    end
  end
end
